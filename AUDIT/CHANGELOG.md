...
- [E-P20] AUTHOR OVERRIDE 2026-09-02: the master prompt's "ND - hard" rule for Calendar sync was explicitly lifted by the author (AC-20 built). New: public/demo/calendar-sync.html (Tier-B Ixana identity, 4 views + params view/ev/try, self-contained 38KB), rewrite in next.config.ts, demoUrl on the card, 4 captures ixana-calsync-*.jpg (1440x900, playwright - scripts/_elev-capture-calsync.mjs), case-study S06 shot+gallery+evidence wired, design note P20 in DESIGN-NOTES.md, recapture-map extended. Demo grounded in the real project source the author supplied at public/demo/Calender-sync-/ (sync engine, loop guard, idempotency, access policy). Internal demo count is now 18 (PROJECT-REFERENCE's 17 is stale on this point).
- [E-P20] playwright added as devDependency for captures (chromium via npx playwright install).
