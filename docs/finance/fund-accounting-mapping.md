# Mapping to fund accounting — Financial Edge NXT as the reference pattern

**Audience:** a finance reader who knows nonprofit fund accounting and wants to know whether the demo's "restricted award" is the real thing or a metaphor. **Purpose:** concept by concept, what the demo has, what the fund-accounting pattern (ASC 958, and Financial Edge NXT as the common implementation of it) calls it, and where the demo stops. Pattern level only: nothing here is a claim about any product's features beyond what the pattern names.
...
| Demo concept | Fund-accounting term (ASC 958 / FE NXT pattern) | In the demo | Where it stops |
|---|---|---|---|
...
| Rules R01-1 … R01-5 | Allowability rules (unallowable costs such as alcohol and entertainment; prior approval for equipment; per-diem; period of performance; indirect base) | Each rule has a positive and a hard negative in the seed and a finding type on the AP agent | Five rules; a sponsor's full cost principles are not modelled |
...
## 3. What FE NXT (the pattern) has that the demo does not
