# User stories — Invoice Watcher and the Tie-out desk
...
Given two invoices from the same vendor with the same invoice number and amount, one paid and one entered later by another analyst · When the watcher runs · Then one finding of type *duplicate-exact* names the later one as its subject, cites both invoices as evidence, and recommends rejecting it — or recovering it as a credit if it was paid. *Test:* `duplicate-exact: raises every planted irregularity and none of its hard negatives`.

**A2 — A recurring bill at the same amount is not a duplicate.**
Given rent invoiced at the same amount every month with a different invoice number · When the watcher runs · Then no duplicate finding names any rent invoice. *Test:* the hard negatives of `duplicate-exact` and `duplicate-fuzzy`.

**A3 — A re-issued invoice with a suffixed number is a fuzzy duplicate.**
Given an invoice re-sent a week later with `-R` appended, the same amount · When the watcher runs · Then a *duplicate-fuzzy* finding names the re-issue, with attributions for amount, days apart and invoice number that sum to one. *Test:* `duplicate-fuzzy …`, `every finding cites … attributions that sum to one`.

**A4 — An unverified bank change before an invoice is raised, high, and the payment is refused.**
Given a vendor whose bank details changed by email the day before its invoice, with no call-back recorded · When the watcher runs and the finance lead tries to release the payment · Then a *bank-detail-change* finding at high severity names the invoice and cites the vendor, and the release is refused with the reason and the call-back to make. *Tests:* `bank-detail-change …`, `refuses … payment to unverified bank details`.

**A5 — A verified change weeks earlier is not raised.**
Given a vendor whose change was verified by phone six weeks before its next invoice · When the watcher runs · Then no finding. *Test:* the hard negatives of `bank-detail-change`.

**A6 — An outlier against the vendor's own history is raised unless a purchase order approved it.**
Given a vendor with four or more prior invoices and a new one at four times their median, with no PO · When the watcher runs · Then an *amount-unusual* finding with the ratio and the robust z-score as attributions; given the same amount on an approved PO for that amount · Then no finding. *Test:* `amount-unusual …`.

**A7 — Orders split under the ceiling are raised as one finding on all of them.**
Given three orders to one vendor by one requester inside a week, each under $5,000 and together over it · When the watcher runs · Then one *split-po* finding whose subject is all three orders and whose evidence includes their invoices. *Test:* `split-po …`.
...
Given an invoice keyed at 23:05 from a long-standing vendor, normal terms, on a PO · When the watcher runs · Then no finding; given one keyed at 22:40 with rush from a two-day-old vendor · Then an *after-hours* finding whose attributions include the partner signals. *Tests:* `after-hours is never raised alone`, `after-hours …`.

**A10 — The watcher never reads the labels and never changes a row.**
Given the dataset · When the watcher runs · Then the ledger hash before equals the hash after, and no module under `agents/` imports `labels.ts`. *Tests:* `no watcher changes the ledger`, `never reads the labels`.

**A11 — A finding that cannot cite a row is never shown.**
Given a finding with an empty evidence list · When results are finalised · Then it is dropped and the drop is counted on the Watchers screen. *Test:* `drops a finding that cannot cite a row`.
...
Given a bank line with no reference, no known counterparty and no amount that matches · Then the line is an exception with the sentence that says which pass failed, and the Tie-out Watcher raises a finding citing the line.
...
Given a Stripe payout · When the workbench recomputes charges − fees − refunds ± adjustments from the balance transactions · Then it agrees with the payout amount and with the bank line carrying the payout id, or the payout is an exception. *Test:* `ties out every Stripe payout …` (seed), demo 29 of 29.
