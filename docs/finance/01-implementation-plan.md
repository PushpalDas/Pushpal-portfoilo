...
| Where | What |
|---|---|
| **Ixana-Wiki** (`PushpalDas/xana`) | The Finance section: nine routes under `/finance`, a Finance icon rail, a `finance/*` mock API, the typed seed, the read-only adapters over Patents and Ops, the five agents, the audit log, the eval harness, the tests. This is the product. |
...
src/lib/finance/
  ...
  labels.ts           planted anomalies (~40) and hard negatives (~120), kept apart from agent inputs
  agents/
    contract.ts       Finding, Draft, TrustLevel, approvals — the one interface every agent returns
    ...
    ask.ts            ask-your-ledger: retrieval over ledger rows, cited answers, refusals
...
src/app/finance/{page,ledger,funds,ap,reconcile,close,health,audit,agents}/page.tsx
...
src/app/api/[...path]/route.ts   + finance/overview, ledger, funds, ap, reconcile, close, health, audit, agents, ask
...
## 4. Adapters and the two reconciliation tests
...
```ts
type Finding = { id; agent; what; why: { evidence: RowRef[]; attributions: {feature; weight; note}[]; rule: string }; confidence: number; severity: 'low'|'medium'|'high'; recommended_action; required_approver: PersonaRole; created_at; }
type Draft   = { id; agent; kind: 'accrual'|'reclass'|'hold'|'match'; payload; finding_id; status: 'proposed'|'approved'|'rejected' }
```

- Agents return only `Finding[]` and `Draft[]`. Every state change (post, pay, accept match, reclassify, release fund, close) is a named human action that writes an `AuditEvent` — tested by asserting the ledger hash is unchanged after every agent run.
...
- AP Anomaly Agent rules (each a pure function with a unit test and a labelled positive + hard negative): duplicate exact, duplicate fuzzy (normalised vendor + amount ± 1 % + 14 days), bank-detail change ≤ 14 days before payment, amount unusual (median/MAD z ≥ 3.5 with n ≥ 5), split POs under the $5,000 ceiling (same vendor/requester, sum ≥ ceiling, ≤ 7 days), missing PO / three-way-match failure, round-number, weekend/after-hours (weight 0.1, never alone), new vendor + rush, off-policy or unallowable-on-fund, misclassification (GL vs vendor category prior).
...
Overview (tiles + "Where this number comes from" drawer on every tile; the patent tile shows $61,900.00 and links to `/patents/spend`), Ledger (virtualised table, filters on every dimension, source chips), AP queue (flags with severity/confidence/evidence/attributions; approving a flagged item needs a reason; SoD refusal when creator = approver), Reconcile (Bank / Card / Stripe payouts; auto-matched, suggested, exceptions; payout = charges − fees − refunds ± adjustments), Funds (utilisation, burn vs period of performance, allowable / questioned / disallowed, reclass proposals needing R-01 PM + Controller), Close (checklist BD+1…BD+5 computed from rows, readiness score from visible weights, `?asof=`), Health (vendor / department budget / chart-of-accounts, 0–100 with inputs), Audit (hash-chained log with a Verify button; auditor read-only), Agents (per agent: queue, last run, findings, acceptance, precision/recall, false-positive trend, trust level, never-does list).
...
| **P2** | Nine routes, rail, nav entry, persona switch, synthetic chip, every view rendering from the seed | All routes 200; screenshots of each view at 1440×900 and 360 px; Patents and Ops screenshots byte-identical before/after; keyboard walk of the ledger and AP queue |
| **P3** | Agent contract, five agents + ask, eval harness, audit log, trust ladder UI | Unit test per rule; ledger hash unchanged after every agent run; uncitable finding dropped (test); SoD refusal (test); audit chain verifies and detects tampering (test); live eval metrics on `/finance/agents` |
...
- **Scope** — nine views, five agents, twelve documents. Mitigation: one shared table/tile/drawer grammar; agents are pure functions over the ledger; documents are written from the same decision log.
