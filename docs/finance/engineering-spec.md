...
seed.ts ──► FinanceDataset ──► views.ts ──► /api/finance/<screen> ──► components/finance/<Screen>.tsx
   │              │
   │              └──► agents/* ──► Finding[] · Draft[] ──► /api/finance/agents, /finance/ap (findings on rows)
   │
...
One process builds the dataset once (`index.ts` `dataset()`), the views compute on every request, the agents run once per (seed, clock, ledger hash) and cache.
...
- `Label { object_type, object_id, anomaly | null, looks_like, note }` — never on the object.
...
- 47 anomalies and 179 hard negatives are planted with `label()`; a row can carry one label per anomaly (D-15). Planted rows are asserted inside the window with `must()`.
...
```ts
Finding { id, agent, rule, subject: RowRef[], what, why: { evidence: RowRef[], attributions: Attribution[], rule }, confidence, severity, recommended_action, required_approver, created_at }
Draft   { id, agent, kind: 'accrual'|'reclass'|'hold'|'match', finding_id, payload, status: 'proposed'|'approved'|'rejected', approvers: PersonaRole[] }
AgentRun { agent, ran_at, findings, drafts, dropped_uncitable, ledger_hash_before, ledger_hash_after, rows_read }
```
...
### 7.2 AP Anomaly Agent (`ap-anomaly.ts`) — the eleven rules
...
### 7.4 Ask-your-ledger (`ask.ts`)

Refusals first (individual pay: pay words plus a person or "per person" phrasing; statutory dates), then twelve intents by regular expression (tape-outs, bank changes, the award, runway, the close, patents, findings, the AP queue, and spend by vendor / project / department / account group / period). Every answer cites up to twelve rows with links; `n_rows` is the full count.

### 7.5 Orchestration (`agents/index.ts`)

`runAllAgents(data)` runs the five, asserts every hash pair equal, and produces one `AuditInput` per run. `agentsView` is the Agents screen's payload including `evaluate(ap.findings, labels())`. `apViewWithFindings` and `auditViewWithAgents` compose the AP queue and the audit log with the runs, so `views.ts` never imports an agent.

## 8. Eval (`eval.ts`)

Per type: positives (labels with anomaly = T), hard negatives (looks_like = T, anomaly null), TP / FN over positives, FP = findings of type T on rows without a T label, FPR = FP on hard negatives ÷ hard negatives; micro-averaged overall; `missed[]` and `unlabelled[]` ids returned so a disagreement can be read. The agent modules do not import `labels.ts` — a test reads their source.
...
`GET /api/finance/{overview, ledger, ap, reconcile, funds, close?asof=, health, audit, agents, personas, ask?q=}`; `api-client.ts` types each fetch by the view's exported interface. The AP payload carries `findings: Record<invoiceId, Finding[]>`.
...
reconciliation (9): Patents and Ops to the cent, nothing after today · seed (6): reproducible, pinned, no clock, labels apart, periods, payouts · ledger (9): closed period, idempotency, correction, payroll suppression, award rules, indirect · agents (26): one per rule, evidence and attributions, after-hours never alone, agents never read labels, uncitable dropped, ledger unchanged by all five, audit chain with runs verifies and detects tampering, SoD refusals, wrong-amount 0 and the one-cent nudge, suggestions below trust, ask refusals and cited answers, eval thresholds, AP queue and Agents view agree.
