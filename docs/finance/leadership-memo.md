# Memo — the Finance orchestrator, and the decision I am asking for

**To:** the founder. **From:** Pushpal. **Purpose:** one page on what has been built, what it proves and does not, and the one decision needed: a read-only pilot on real exports. **Every number below is from a synthetic ledger and says so.**
...
Anything about our real ledger. The rules were tuned on the seed they are scored against; the first run of the eval read 0.34 precision and most of that was the labels being incomplete, which is the kind of thing a real month will do again. Acceptance is n = 0 because no person has yet accepted or dismissed a finding. The business case is modelled, not measured, and says so on every line.
