...
| `prng.ts`, `dates.ts`, `hash.ts` | mulberry32 seeded 20260925; calendar helpers (business days, period end, Fridays); FNV-1a fingerprint + stable JSON for the audit chain |
...
| `labels.ts` | The ground truth, exported apart from `index.ts` so agents cannot import it |
...
```
✓ tests/finance/reconciliation.test.ts (8)
...
✓ tests/finance/seed.test.ts (6)
    ...
    seed.ts never reads the clock; 43 anomalies / 184 hard negatives, no row carries a label;
...
| Planted | 43 anomalies: duplicate-exact 4 · duplicate-fuzzy 4 · bank-detail-change 3 · amount-unusual 5 · split-po 7 (three splits) · missing-po 5 · round-number 3 · after-hours 2 · new-vendor-rush 2 · off-policy 4 · misclassification 4 — and 184 hard negatives |
