...
|---|---|---|
| `prd.md` | requirements | six personas and their permissions, five goals, the non-goals as a contract, FR-1 … FR-15 by screen and agent, quality bars, pilot success measures with thresholds agreed before build, open questions |
| `engineering-spec.md` | engineering | the module map, the data model, the seed and its invariants, both adapters with their tests, the ledger, the views, the contract, the eleven rules as a table (reads / raises when / never when), the other four agents, ask, the eval, the audit chain, API and client, components, the 50 tests, the invariants list |
| `user-stories.md` | acceptance | 13 Given/When/Then for the AP Anomaly Agent and 10 for the Reconciliation workbench, each naming its test or marked *pilot* |
...
| `fund-accounting-mapping.md` | domain | concept by concept against ASC 958 / the Financial Edge NXT pattern, what the pattern has that the demo does not, what the demo has that the pattern leaves to people, what a real award would need |
...
- **Demo path** (D-18): `/demo/finance-desk` and `/demo/finance-desk/:path*` **redirect** to the Wiki's `/finance…` with the query preserved; a cross-origin rewrite would have served the Wiki's page with this site's asset paths. The Wiki's `/finance` hops `?view=<screen>` to `/finance/<screen>`; the AP queue takes `?try=approve|pay` so the SoD image is the state its link opens.
...
Eight at 1440×900 from the dev server, each the state its link opens: overview (card + evidence), `ap?id=INV-0236` (shot), `ap?as=dana&id=INV-0254&try=approve`, `reconcile?tab=bank&filter=suggested`, `funds?fund=award-r01&verdict=unallowable`, `close?asof=2026-08-05`, `agents`, `audit`. Zero page or console errors during capture.
