 ...
 * /lets-connect shows one implementation rather than two that
...
export default function ConnectForm() {
	...
	useEffect(() => {
		...
		const ctx = gsap.context(() => {
			// fromTo, not from: `from` writes the hidden start state immediately
			// and only ever undoes it by playing. If the tween never advances —
			// the page opened in a background tab, where rAF is throttled — the
			// form and the social cards stay at opacity 0 for good. fromTo plus
			// the safety net below means the end state is reached either way.
			tl = gsap.timeline({ defaults: { ease: 'power3.out' } });

			tl.fromTo(
				formRef.current,
				{ y: 60, opacity: 0 },
				{ y: 0, opacity: 1, duration: 1, delay: 0.6, clearProps: 'opacity' },
			).fromTo(
				'.lc-social-card',
				{ y: 40, opacity: 0 },
				{
		...
		// Belt and braces: if the entrance has not finished by the time it
		// should have, jump it to the end rather than leave anything hidden.
		const settle = window.setTimeout(() => {
			if (tl && tl.progress() < 1) tl.progress(1);
		}, 3000);

		const onVisible = () => {
			if (!document.hidden && tl && tl.progress() < 1) tl.progress(1);
		};
