...
export default function ExperienceList({ items, setModal }: ExperienceListProps) {
    const listRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        const ctx = gsap.context(() => {
            // Animate stripes scaling in
            const stripes = listRef.current?.querySelectorAll('.experience-list-stripe');
            if (stripes) {
                gsap.from(stripes, {
                    scaleX: 0,
            ...
            const rows = listRef.current?.querySelectorAll('.experience-list-item-wrap');
            if (rows) {
            ...
            <div className="experience-list-container">
                ...
                <ul className="experience-list-items">
                    {items.map((item, index) => (
                        <li key={item.title} className="experience-list-item-wrap" style={{ opacity: 0.2 }}>
