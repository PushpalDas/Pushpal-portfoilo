...
const getTimelineRange = (items: ExperienceItem[]) => {
	const allDates = items.flatMap(item => [item.startDate, item.endDate || 'present']);
	...
	const hasPartialFinalYear = dates.some(d => d.getFullYear() === maxYearExact && d.getMonth() > 0);
		...
		<div className="experience-timeline-section" ref={containerRef}>
			<div className="timeline-container">
				...
				<div className="timeline-horizontal">
						<div className="timeline-row" style={{ alignItems: 'end', marginBottom: '1rem' }}>
							<div className="timeline-row-title"></div>
							<div className="years-axis">
								{Array.from({ length: maxYear - minYear + 1 }, (_, i) => minYear + i).map((year) => {
									const pos = ((year - minYear) / (maxYear - minYear)) * 100;
									return (
										<div key={year} className="year-label" style={{ left: `${pos}%` }}>
											{year}
										</div>
									);
								})}
						...
						<div className="timeline-bars-container">
							{sortedItems.map((item, idx) => {
								...
								const endPos = getTimelinePosition(item.endDate || 'present', items);
								...
								return (
									...
									>
										...
										<div className="timeline-bars-track">
											...
											>
												{width >= 40 && <span className="bar-designation inside">{item.title}</span>}
												{width < 40 && (
													<span
														className="bar-designation outside"
														style={
															startPos > 50
																? { right: '100%', left: 'auto', textAlign: 'right', paddingRight: '0.8rem' }
																: { left: '100%', right: 'auto', textAlign: 'left', paddingLeft: '0.8rem' }
														}
													>
														{item.title}
													</span>
												...
												<div className="bar-label">
													<span className="bar-label-dates">
														{item.startDate}
														{item.endDate && item.endDate !== 'present' && ` - ${item.endDate}`}
														{item.endDate === 'present' && ' - Now'}
													</span>
													<span className="bar-label-services">{item.services}</span>
												</div>
											</motion.div>
										</div>
