...
function CalendarIcon() {
  return (
    <svg className="cs-meta-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
function DriveIcon() {
  return (
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <polygon points="1 6 1 22 8 22 12 15 8 6 1 6" /><polygon points="17 6 11 6 7 22 14 22 23 6 17 6" /><polygon points="12 1 8 6 16 6 20 1 12 1" />
    </svg>
  );
}
...
function MediaSlot({ item }: { item: MediaItem }) {
  ...
  if (item.type === 'image' || item.type === 'gif') {
    inner = (
      ...
      <img
        src={item.src.startsWith('http') || item.src.startsWith('/') ? item.src : `/static/images/project/${item.src}`}
        alt={item.caption ?? ''}
    ...
    inner = (
      <iframe className="cs-media-iframe" src={item.src} title={item.caption ?? 'Demo'} />
    );
  }
    ...
    <div className={item.fullWidth ? 'cs-media-slot--full' : undefined}>
      ...
      {item.type === 'demo' && (
        <div style={{ marginTop: 8 }}>
          <a href={item.src} target="_blank" rel="noopener noreferrer" style={{ color: '#4a7aff', fontSize: 13 }}>
            Open in new tab ↗
          </a>
        </div>
      )}
...
export default function CaseStudyPage({ data }: Props) {
  ...
  const links = data.links
    ? data.links.filter((l) => l.label && l.label.trim() !== '' && l.url && l.url.trim() !== '')
    : [];
      ...
      <nav className="cs-topbar">
        <div className="cs-topbar-inner">
          <Link href="/work" className="cs-back-btn">
            <span className="cs-back-arrow"><ArrowLeft /></span>
            Back to work
          </Link>
        </div>
      </nav>
      ...
      <div className="cs-content">
        ...
        <header className="cs-hero cs-reveal">
          ...
          <h1 className="cs-title">
            {isEmpty(data.title) ? <span className="cs-placeholder">[Your project title]</span> : data.title}
          </h1>
        ...
        <div className="cs-bottom-nav">
          {data.prevProject ? (
            <Link href={`/work/${data.prevProject.slug}`} className="cs-nav-link cs-nav-link--prev">
              <span>← Previous project</span>
              <span className="cs-nav-link-title">{data.prevProject.title}</span>
            </Link>
          ) : (
          ...
          {data.nextProject ? (
            <Link href={`/work/${data.nextProject.slug}`} className="cs-nav-link cs-nav-link--next">
              <span>Next project →</span>
              <span className="cs-nav-link-title">{data.nextProject.title}</span>
            </Link>
          ) : (
            <div className="cs-nav-empty" />
          )}
