...
export default function CertificationsList({ items, setModal }: CertificationsListProps) {
    const listRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        const ctx = gsap.context(() => {
            // Animate stripes scaling in
            const stripes = listRef.current?.querySelectorAll('.certifications-list-stripe');
            if (stripes) {
                gsap.from(stripes, {
                    scaleX: 0,
            ...
            const rows = listRef.current?.querySelectorAll('.certifications-list-item-wrap');
            if (rows) {
            ...
            <div className="certifications-list-container">
                ...
                <ul className="certifications-list-items">
                    {items.map((item, index) => (
                        <li key={item.title} className="certifications-list-item-wrap" style={{ opacity: 0.2 }}>
