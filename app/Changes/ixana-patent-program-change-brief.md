...
## 15. Change record — 2026-09-26, the spend surface becomes the ten-view tool

**What prompted it.** The five-view `patent_spend-1.html` turned out to be identical to what the demo already ran (§14). The genuinely newer tool is `patent_spend_v2.html` (Cost analysis/output, 18 Sep 2026): ten views over an invoice-line ledger with quotes, variance, routes, a forward budget, a negotiation sheet, a firm comparison and exceptions. The user chose to port it.

**Confidentiality first.** The v2 file carries real firm names, real quotes, real matter titles, application numbers and every real invoice line, plus a person's name in its open questions. **None of it was copied.** The port follows §12 Route A in spirit — the tool's logic and layout, a synthetic dataset — with the dataset built as a *derivation*: `src/lib/patent-ledger.ts` turns the same 18-matter `TIMELINE` every other surface reads into 70 invoice lines (one per payment; the 16 undated payments now carry the date of the filing they were invoiced with, so every line has an invoice number — event count 115 → 131, totals unchanged at $61,900 / 70 payments). The only things written down, all invented: a fee taxonomy shaped like the tracker's, three firms' quote schedules (`QUOTES`), the reasons per fee type, docketed national-phase deadlines for the six PCT matters, forward-budget quotations, four exceptions, three open questions and eight quoted-never-invoiced items. The regression grep (§13) returns 0 on every file.

**The thesis held.** v2 computes 30/31-month national-phase deadlines from priority dates. This page's spine is *no statutory date computed anywhere*, so the demo stores each deadline as the firm's docket entry (`DOCKETED_NATIONAL_PHASE`, with `docketed_by`) and says "mirrored, not computed" wherever one is shown — Gantt diamond, Changes, Routes, drill. The forward budget is labelled ESTIMATE and states that no matter has entered a national phase (n = 0 observed), so its entry costs are the firm's quotations, not history.

**Demo (XANA `main`).** `PatentSpend/` rewritten: `ledger.ts` (filters, dimensions, KPIs), `SpendChart` (colour by category / jurisdiction / owner, tracker-order and lines sorts, ▲ above-quote flag, PCT chip, variance under each total, projected cost-to-grant overlay, click → drill), `SpendDrill` (matter → category → fee type → line with the why), `SpendMatrix` (US lifecycle and PCT + national phase, per-firm blocks with quoted price, totals at quote, variance $ and %, above/below), `SpendTable` (the ledger, sortable, Export CSV), `SpendTimeline` (spend by month + invoice/event feed), `SpendGantt` (whole-programme lanes, docketed deadline diamonds, today rule, click → drill), `SpendChanges` (OA without response, allowance without issue fee, deadlines inside 400 days, HIGH exceptions, plus the dashboard's internal action dates kept apart), `SpendRoutes`, `SpendNegotiation`, `SpendFirms`, `SpendExceptions`. Header: search, Export CSV, five KPIs (matters with spend · law-firm vs government · never quoted · actual vs quoted · most expensive), period defaulting to all time, firm, route and colour selectors. Deep links: `view`, `range`, `firm`, `route`, `color`, `sort`, `mx`, `q`, `proj`, `drill`. The payload gains `ledger` (lines, patents, fee types, stats, quotes, exceptions, questions, model, e_oa, recon). Type check and lint clean; ten views screenshotted locally with zero page errors.

**Case study.** Gallery tile 1 recaptured from the new landing state (`ixana-patent-spend.jpg`, same script); label, caption and alt rewritten for the ten views; surface-table row renamed *Spend, reasons and routes*; `config` describes the ledger, the variance rule, the mirrored deadlines and the estimate label. **No prose changed**; the row stays at 991 words.

**Known departure from v2.** No μ/σ (v2.2 removed them too); no national-phase actuals (none exist on the synthetic portfolio); the negotiation sheet targets Firm A only, as v2 targets one firm.
