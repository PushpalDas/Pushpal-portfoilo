...
A second spin is the easiest programme in the world to justify badly: everyone has a change they want, the die is proven, and the argument for going again is always available. The question was not whether to spin but what the spin was for — a gen-2 that absorbs every request is a gen-1 arriving eighteen months late.
...
I collected every change request, field issue and evaluation complaint against YR23 — 63 — and ranked each by two defensible numbers: stalled evaluations unblocked, and cost in die area and validation weeks. A list of 63 requests is an argument for a third spin; a ranking is a decision.
...
The routing was deliberate. Field issues, evaluation complaints and lot data arrived through three channels to three audiences, and nobody scored them against each other — so I ran them into one queue and made the ranking the artefact everyone argued about.
...
The backward-compatible energy mode was the hardest cut. Partners with a shipping YR23 design wanted YR31 to run 5 Mbit/s at gen-1’s exact 0.12 nJ/bit — drop it in now, inherit the rate later, power budget untouched. It needed two separately tuned analogue front ends on one die: the area was findable, the validation was not, since every mode multiplies the characterisation matrix the spin had just reduced. The gap is covered twice — interop parity, and YR23 staying in production.

#### 05. How I got it agreed

The test lead objected to cutting the ATE program — the strongest objection anyone put to me on this programme. Test time at volume is a real line in a customer’s cost model, so pressure to cut it is permanent, and every test removed is a class of defect that now reaches a wearable on somebody’s head. A field failure is not a return; it is a design-in at risk.

I had opened by asking for a test-time target, which invited exactly the argument we were having. So I withdrew it and asked which tests had actually failed a unit across every YR23 lot. Roughly a third had never fired. That moved the conversation from how much test we could afford to which tests had earned their seconds — his question to answer, not mine.
...
Three ways of getting Wi-R into partner hands were on the table for FY25: an onsite bring-up by our applications engineer, a full reference design, or a dev kit. The commercial stake was concrete — our largest pipeline evaluation, a hearables OEM weighing Wi-R against BLE for an earbud-to-case link, had said plainly they would not staff an RF engineer to evaluate an unproven physical layer. If evaluation stayed expensive we lost the account, and the reference design every later hearables conversation would lean on.
...
I did not think we would win on principle, so I re-grounded it in the pipeline we had. Every completed evaluation had asked a variant of “does this link hold on a person” before any productisation question — and bundling would put a two-month reference-design lead time in front of a question we could answer in a day. Deal size stopped being the argument; what the partner is trying to learn first became it.

He agreed, with two conditions I had not planned to give. The kit ships with two worked examples rather than one, demonstrating a use case and not just a link — three weeks of firmware time. And any partner completing a kit evaluation gets the reference-design files with no separate commercial step, so the kit never becomes a toll gate — which is why eval-to-design conversion is measurable at all.

#### 06. What was built

A sealed kit: a Wi-R transceiver module, a known-good host board, two tested electrode assemblies, a USB console and two worked applications. A partner engineer opens it, mounts the electrode pair as printed on the enclosure, connects the console, and watches a link establish across their own body inside the hour. Everything that could have been a variable — spacing, contact material, ground reference, host clocking — is fixed and characterised.
...
I measured the kit and never measured the evaluation. Time to first link was right for the hardware and wrong for the business: it says the box works, nothing about whether the partner reached a decision. Nine of 34 converted, and I cannot say cleanly why the other 25 stopped — we instrumented the first hour thoroughly and the following two months not at all.
...
**Result.** Fabricated citations cannot ship: any reference the retriever did not return is stripped by code before an answer renders, and out-of-corpus questions are refused by a calibrated gate rather than answered fluently. Engine and interface both work and are not yet connected, so nobody has used it; the evaluation figures below are the worked example.
...
Refusal is the feature. Reframing from “how do we always answer” to “how do we reliably decline” made the rest of the design follow.

#### 02. The problem as people experienced it

We collected 100 real questions from community forums rather than writing our own, then had three reviewers who know the corpus independently judge whether a returned passage addressed each. Real questions matter: the ones people ask are vaguer than any test set we would have written.
...
- **A deterministic citation guard, not a prompt instruction.** Any citation not in the retrieved set is stripped by code. Asking a model not to fabricate reduces the rate; a deterministic check takes it to zero — only one of those is a guarantee.

#### 04. What was built

A retrieval engine over books, letters and verified lecture transcripts, held as roughly 32,000 chunks. A question is embedded, candidates come from the book and lecture lanes separately, a re-ranker reorders them, and a calibrated distance gate refuses what the corpus does not address. A deterministic guard removes any cited reference that was not retrieved. The interface was prototyped separately; the two halves are not yet connected.
...
We built an engine and an interface and never connected them — the fact that defines where this project stands. Both halves are good and neither has met a user, because each was sequenced to be finished rather than to meet the other early. A crude API between two half-built halves would have produced worse demos and better information.
...
**What I did.** I ran delivery of first silicon across five functions and two sites. The founder owned the product bets; I owned the system that turned them into a shippable part on a date — first silicon’s scope, what shipped as errata against what forced a respin, and the launch gates.
...
The commercial stake was specific: a hearables OEM and an AR-wearables maker had both seen Wi-R work on a bench and asked the same question — what do we put in the schematic. Neither would staff a program against a research result, and if the answer stayed “a demo and a paper” we lost the first design-ins.
...
That gap is why a part was worth building rather than a paper worth citing, and why first silicon had to be honest about the whole ladder: an energy claim at one operating point is a headline; a design-in needs the number at the rate the product will run.

#### 02. The problem as people experienced it

Rather than ask engineers how bring-up went, I audited the bench logs from the first 24 evaluation boards on first silicon and coded every stall by cause. The logs were kept for debugging, not for me — which is what makes them worth reading.
...
I should be precise about what I owned, because a title on a silicon program is easy to inflate. The founder owned the product bets — that Wi-R should exist, which market, what the part is for. I owned the delivery system that turned bets into a shippable part on a date: five functions, two sites, one schedule, and the gates that decided what shipped.

That system is where delivery fed product. Validation capacity set the tapeout date; tolerable escapes set what first silicon could claim; whether a workaround fit in firmware decided whether a feature shipped at all.
...
On-die link encryption was the hardest cut. It failed on two constraints at once: die area against a closed floorplan, and a validation matrix with no bench hours before the committed tapeout. Wi-R’s security position is physical — the field stays within about 0.1 m of the body, so there is no signal at 5 m to intercept — and the reference driver encrypts host-side where compliance needs it. The cost is that anyone needing crypto in silicon waits for a later part.

#### 05. How I got it agreed

The post-silicon validation lead wanted to hold first silicon for a respin rather than release with documented errata and firmware workarounds. His argument was not caution for its own sake: a part that needs a workaround gets designed in wrong by somebody who never opened the errata sheet, and the silicon gets the blame.

He was right about how errata sheets get read, so I stopped arguing schedule and split the 41 escapes into two kinds: those that change what the part can do, and those that change how you must drive it. Only three were the first kind, none in the launch segments’ paths. That turned risk appetite into a question about three specific defects.

He agreed on two conditions. Every workaround ships as code in the reference driver rather than a note in a PDF — a partner cannot design it in wrong by skipping the document; six firmware weeks, and the best thing on this page. And any later escape in a launch-segment path reopened the respin conversation, him holding the pen.
...
Before tapeout we agreed first silicon failed on one condition: if it could not hold a sustained on-body link at full rate on an ordinary person without a metal fixture, it went back for a respin before any customer saw it. Interop was the guardrail — the date could not be won by a part that only worked on our reference board.
...
**How we counted.** An escape is an issue found after tapeout that a pre-tapeout check should have caught, logged whether or not it turned out to be the die. Blocking means it could not be closed in firmware or documented as a workaround at triage — the three that carried the label were exactly that when triaged, and later driver work closed all three, the reclassification the summary counts. Board and setup problems stay in deliberately: 26 of the 41 were never silicon, and excluding them would hide where bring-up time went. The four OEM programs counted here are pre-kit direct engagements, not the dev-kit page’s kit-mediated design-ins.
...
I scheduled the die and left the interface around it to look after itself. Every expensive surprise in section 02 came from the electrode interface, the one part of the system with no owner and no line on the schedule — not a design deliverable, not a validation deliverable, so it belonged to nobody.
...
**Result.** Real throughput went from about 3.4 to 13.5 Mbit/s — the announcement’s own figures — putting a 100 MB payload inside a one-minute service window. Gen-1 stayed in production for designs needing its longer reach, so the upgrade stranded nobody. The pipeline that pulled this spin, and what converted, is the worked example below.
...
The case for this spin was unusual: I did not have to make it. Fourteen evaluations had already written down what they needed and told us we did not meet it — demand documented, quantified and sitting in a folder. My job was to stop us spending it on something else.
...
Timing mattered too: sealed enclosures were becoming the default across industrial and medical hardware just as firmware images crossed a hundred megabytes, so the gap between what a near-field link could carry and what a service workflow needed was widening on its own.

#### 02. The problem as people experienced it

Fourteen evaluations had supplied a written requirement sheet before stalling, so rather than interview anyone I put those sheets beside our gen-1 characterisation data and scored each line met or missed. Requirement sheets are the most honest artefact a partner produces: written to be held to, before anyone is invested in the answer.
...
On this programme the delivery loop did the product definition outright. The founder owned whether the near-field line deserved a second part; I ran the review that decided what it had to be, converting the gap between fourteen requirement sheets and our characterisation data into a specification the design team could work from.
...
Operation through a fully conductive enclosure was the hardest thing to refuse, and the most asked for — industrial partners with metal housings wanted the sealed-service story their plastic-cased neighbours were getting. It is not a scheduling constraint: an enclosed conductive shield blocks electric-field coupling, so the only route was a second inductive front end beside the electric one — die area, a second validation campaign, and the RF-silent property that is why these buyers are here. Enclosure-material guidance and an aperture pattern in the reference layout cover the partly-conductive cases; the datasheet says plainly a full Faraday cage is out of scope.
...
Arguing about which market was larger was unwinnable — the honest answer is his. So I compared the quality of the two numbers instead. The pipeline 480 thousand were written by partners into their own requirement sheets after qualifying the part; the derivative 200 thousand were ours. Which opportunity is bigger became which number we would still believe in a year.

He agreed, and attached a condition. The gen-2 spin had to carry the hooks the derivative would need — a documented reduced-rate, low-power operating point selectable at provisioning — so the derivative becomes a binning decision rather than a third spin. It cost unbudgeted validation weeks, and it is why a derivative is now a quarter of work instead of a year.

#### 06. What was built

A near-field transceiver moving about 13.5 Mbit/s of real data at 1–15 cm, under 6 mW, sub-0.2 ms, with no radiated RF. In workflow terms: 25 MB in about fifteen seconds, 50 MB in thirty, and a 100 MB firmware image plus validation logs in fifty-nine — the number that decides whether a sealed device can be serviced unopened.
...
**What I did.** The founder owned the bet that a near-field line should exist. I owned turning it into a delivered part while the BAN program kept its dates: shared validation capacity, a scope built for a different buyer, and launch gates for a part that had to work on hardware we did not design.

**Result.** XA-NFE2001 launched at 5 Mbit/s under a milliwatt — over 50× NFC’s energy efficiency and ten times its throughput, per the published comparison — with the physics written up in an arXiv paper. It shipped as a second line while BAN kept its dates; the interop matrix that qualified it is the worked example below.
...
The commercial question was whether Ixana was a one-line company. A single line makes every investor and large-customer conversation about concentration risk, and puts one thing on the roadmap. A second line answers both — and a second line that slips adds an execution problem to the concentration risk.
...
The split is the same as on the BAN program: the founder owned the bet that a near-field line should exist and what it was for; I owned the delivery system that made it a part — the same five functions across two sites, now running two silicon programs against one pool of validation capacity.

Running two lines is where delivery fed product hardest. Shared validation capacity forced the range decision below, and the interop matrix we could afford set how configurable the part could be. Those were settled as scheduling questions, and they shaped the datasheet.
...
Runtime-adjustable range was the hardest cut, and the one partners asked about most. A continuous knob the host could turn is obviously useful — tight for a payment, open for a bench transfer. It failed on validation capacity already split with BAN: every additional setting multiplies the interop matrix, and we could afford twelve configurations, not sixty. Three profiles cover the cases the inbound requests described; a partner needing a fourth gets it characterised in a later revision rather than finding at integration that nobody tested theirs.

#### 05. How I got it agreed

The COO would not commit a build plan for a second line on gen-1 demand signal. His objection was capital: committing wafer starts and package tooling for a part with no shipped revenue, on the strength of inbound emails, looks obvious beforehand and reckless in a quarter where BAN needs the same money.

I had been arguing that the demand was real, which is unfalsifiable and therefore useless. So I made it falsifiable: we converted the 96 coded requests into a sample-request commitment — before any long-lead spend, that population would formally request samples against a spec sheet with a price on it. Interest that survives a form and a price is a different signal.

104 requests came back — eight of the 96 companies asked from two teams, which is how 96 coded requests produce 104. He committed the build plan, with a condition I had not planned to offer: the second line gets its own line in the capital plan, so if NFE underperforms it is visible as NFE underperforming. Every subsequent conversation about a third line has been shorter for it.
...
Before the build plan was committed we agreed the line failed on one condition: if the part could not hold a link across the majority of host and range configurations on hardware we did not design, it was a demo, not a product. The BAN program was the guardrail — a missed BAN milestone while the second line spun up meant NFE was being paid for out of the first.
...
I let the interop matrix be sized by what we could afford rather than where risk was. Twelve configurations was a capacity number, not an analysis, and the cell that failed was predictable from first principles — slowest host, longest range. An hour of thinking would have spent the same twelve slots on the twelve riskiest combinations instead of a tidy four-by-three grid.
...
**What I did.** I scoped a simulator around one claim: watching the Bloch sphere move as you place a gate teaches something a table of amplitudes does not. That meant a hard qubit cap so rendering stayed instantaneous, and an interactive grid rather than a code editor.
...
I sat in on two teaching sessions and surveyed 31 learners on what they could do versus what they could picture. The survey mattered more, because the gap is internal — a learner getting the right answer looks identical to one who understands it.
...
I set the scope, the qubit ceiling and the boundary between numerical engine and rendering layer, and ran the milestones across both.
...
A web application for building, simulating and visualising quantum circuits. Gates land on a timeline grid; the engine computes state vectors, probabilities, shot distributions and Bloch coordinates per qubit; the front end renders interactive 3D spheres that update as the circuit changes. A prebuilt library loads standard algorithms. It is complete, and no learner has used it.
...
I built the whole thing before showing it to a single learner. A paper prototype of one Bloch sphere responding to one gate would have tested the core claim in an afternoon; I spent five months on an engine, a renderer and a circuit library to test it properly. The completeness is why it never shipped — it kept being nearly ready.
...
**What I did.** I owned WishKey end to end as an engineer: an electronic cabinet where each slot locks and reports independently, so a key cannot leave without an event that names an authenticated person. The carrying design decision is per-slot locking rather than one locked door.

**Result.** A key cannot leave without an event naming an authenticated person, because the record is a side effect of the act rather than a form beside it. It ships as a catalogue product through installer partners, fully useful with nothing else installed. The deployment and audit figures below are the worked example.
...
The cost of a lost key is never the key. It is rekeying every lock that key opened, the site running degraded until that finishes, and an audit finding saying you could not name who held it. The segments carrying this hardest are those where a key opens something regulated.
...
I interviewed facility managers and the installers who service their sites, then sat with twelve months of paper key-register pages from six pilot sites, counting rows that could not be resolved to a person, a key, or a return.
...
My title was senior embedded design engineer. The CTO owned the commercial relationships — the installer channel, pricing, which enquiries we chased — and I owned the product from requirements through schematic, board, firmware, enclosure and production handoff. The product-definition work was mine without the title.

The decision everything else follows from is per-slot locking. A cabinet with one locked door is far cheaper and gives you a door audit: a person opened the cabinet — not which key left with them.
...
Fingerprint authentication was the hardest cut and the one every prospect asked about. It failed on the population: our sites run shift workers and contractors with dirty, wet or damaged fingertips, often gloved, and a false reject at the cabinet does not degrade gracefully — it sends someone to the mechanical override or to propping the door, precisely the behaviour we were selling against. An employee RFID card plus PIN covers the gap, with the anti-smash seal so a returned tag cannot be a decoy. The honest cost: the trail attributes to a card and a PIN, not a face.

#### 05. How I got it agreed

The installer partner who would commission these at customer sites read my prerequisites list and refused to carry the product — correctly. The list asked the customer for a Linux server, two network points, directory synchronisation, an SSO endpoint and a mail relay before a cabinet would work. His crews are electricians: every line was a site visit ending in a call to somebody’s IT department, and the delay would be blamed on the box with our name on it.

I had been defending the list feature by feature — the wrong argument, because each item genuinely earned its place. His frame was better: not whether an integration is valuable, but what the cabinet must do the day it is bolted to a wall. I recut the requirements so full audit capability depends on nothing outside the cabinet; every integration became optional and post-commissioning.

He agreed, then asked for something I had not planned. Commissioning had to be signable on the spot, so the cabinet itself produces the acceptance report his foreman hands the customer, from its own local store, before any server exists. We permanently carry a local log and report generator duplicating the server — a standing maintenance cost, and the two drift if nobody watches.
...
We agreed before build that this failed if any key could leave a cabinet without an event record naming either an authenticated user or a witnessed override. Unattributed removals are the guardrail rather than the headline, because everything else here improves by making release easier, and easier release is how you lose the record.
...
**How we counted.** Unaccounted-key rate is the share of key-removal events with no attributable authenticated user. Emergency mechanical overrides are excluded — recorded, witness-signed and reported as a tile above rather than a footnote, because excluding a category quietly is how a guardrail stops meaning anything. Late returns count as attributed: we know who has the key, a different failure from not knowing. Keys under management counts keys, not slots — a slot holds one tagged bunch averaging about five keys, which is how 3,420 keys sit behind 660 locks across 22 cabinets. Mechanically, an unattributed removal is a slot reporting tag-out with no authenticated session open — forced, or released in a fault window — logged with timestamp and slot but no user to charge.
...
I treated late returns as somebody else’s problem because the guardrail was attribution, and the chart above is the bill. The cabinet knows who holds every overdue key and does nothing beyond a report a manager has to go and read. A nudge when a key falls due was small work I never scheduled, because it did not fit the metric I had chosen.

I would also have made the mechanical override self-evidencing rather than procedural. Today it is a real key and a witness signature on paper — the exact weakness the product exists to remove. A seal on the override lock, logged when broken, would have cost little and closed the last honest gap in the trail.
...
By late 2024 the technology objection was answered: the BAN part in production, the near-field part close behind, the dev kit reliably producing a working link in a day. What had not moved was conversion — teams impressed in week one were quiet by week ten, consistently enough to be structural.
...
This is the program where a delivery lens most visibly turns into a product one. The founder owned which markets mattered and why Wi-R belonged there. I owned what the designs became: which two verticals we built, what “done” meant for a design an OEM could copy, and when it was allowed out.
...
He agreed, with one condition. Every released package ships with the exact validated BOM including tested substitutions, and any change forces a versioned re-release rather than a quiet file swap — a partner building from a stale layout is a support incident that looks like a silicon fault. The obligation is permanent, and it has no owner outside my program.
...
**How we counted.** An active evaluation is one where a named engineer has requested design files and has hardware on order or in hand. Teams holding a dev kit without a project, or that only asked for pricing, are excluded — both were counted before this program, inflating the number while explaining none of the conversion. The nine period design-ins are the same nine on the dev-kit page; six cite a reference design, so the figures overlap by construction and must not be added.
...
I gated release on an outside rebuild, then chose the outside team from people who already liked us. Both first rebuilds succeeded — which felt like validation and was closer to a formality, because a team already invested pushes through ambiguity a neutral one would file as a defect. The gate was right, the sample flattering; the rebuild should come from an evaluation not yet committed.
...
The reframe: the tier is not competing on watches, it is competing on shop windows. A colour face sells the unit, then spends two years disappointing the person who bought it.
...
The colour screen was the cut that set the tier, and what it really removed was the shop-window demo. A reflective display is unreadable in the one place a buyer sees it — a lit cabinet, held for thirty seconds — and unbeatable everywhere they will actually use it. What covers the gap is the number we could print: nine days on a charge, on a stated profile, against a tier that claims a week and delivers three days. The honest cost: we depend on the box, the listing and the reviews to make an argument the product cannot make by being picked up — in a retail aisle we lose to a brighter rectangle.
...
I had been defending the choice on power and readability — answering a question he had not asked. So I took his frame: the risk is not which display is best but what the product does on the day that display is unavailable — and moved the decision up a level, specifying the interface instead of the part: a defined connector and pin map at the board edge, and a display abstraction in firmware.
...
I made warranty returns the guardrail and built no way to read a watch that came back. No cycle count, no reset counter, nothing stored on the unit — return analysis is a spreadsheet of what customers wrote in a form and what a technician guessed. A few bytes of flash would have turned the number I chose to be judged on into something I could act on.

I also accepted the charging cradle. The teardowns told me a proprietary connector generates returns in this tier, I wrote it down, and shipped one anyway — the cradle was tooled before I got to it, and arguing meant reopening a decision outside my scope. It was inside the scope I claim on this page, and I did not fight for it.
...
Ranking the five teams was the hardest cut, because leadership asked for it directly. It fails on comparability, not cost: post-silicon is bench hours, firmware is release cycles, hardware is board spins around a fabrication slot. A number meaning "late" in one does not mean "late" in another, so any ordering is an artefact of the vocabulary we agreed rather than a fact about delivery. What covers the gap is a portfolio sheet comparing coverage — how much of what a report claims carries a name — the same measurement everywhere. The cost is that the question is still asked, and answered in a room where nobody can check it.
...
One n8n sub-workflow that takes a team and a window. It clears prior rows, fetches tasks due inside the window, and runs parallel branches — efficiency per assignee, group distribution, and delayed-task analysis that pulls each comment history and drafts a cause. A second pass reads the generated sheets, not the source — why the narrative and the tables agree. At the quarter nothing is fetched.
...
Ixana already had a read-only dashboard over this workspace, and read-only was right for it. Ceremonies were the part it deliberately did not touch, so they stayed in documents beside the workspace, updated by whoever remembered.
...
- **Write-narrow, rather than read-only.** A ceremony exists to change state, and a read-only one sends people to a second tab — where the hand-maintained document came from. This writes: four fields, one module, every write listed.
...
She agreed, with two conditions: no per-person export of any kind, and a ceremony archive visible to every member of a squad rather than its lead, so anyone who can be measured can read the measure. The second is why the write ledger is a product surface and not a log file.

#### 06. What was built

Four ceremonies over one dataset. The standup board renders the record’s diff since yesterday, blockers attached to tasks. Planning shows the ranked backlog beside a capacity meter built from each engineer’s median throughput. The burndown sums task rows on read, in points and tasks, so slicing shows as two lines separating. Behind them: the drift check and the write ledger.
...
The guardrail was the median size of a closed task: a burndown counted in tasks rewards closing small ones, and a product improving every meeting metric while halving task size is a worse product with a better dashboard. It has held at three points since launch.
...
Drift stands at zero, and I do not report it as an achievement: it is zero because the second copy no longer exists — which is what removing it means.
...
And I would have solved the retrospective instead of cutting it. The cut was correct on the constraint I set, but I treated the constraint as fixed rather than the thing to solve. A genuinely anonymous surface returning only action items would probably have worked.
...
**Result.** Each team kept its own stage vocabulary — the canonical model would have been wrong for two of three — and classification ships with its correction rate on the header, where the people relying on it can see it. The assistant refuses the ranking question outright. The adoption and preparation-time figures below are the worked example.
...
It was urgent rather than merely large: a delay nobody explains gets re-explained at the next review, so the two largest bars fed each other week after week.
...
The reframe: leads were not short of data, they were short of an agreed shape for it. Three teams with different flows had been asked to fit one export format, so every review began by undoing it.

#### 03. My role and approach

I owned scope across ingestion, the pipeline model, the two views and the assistant — and the negotiation with each team about what their stages were. That negotiation was the product work.

**Key decisions**

- **Three pipeline definitions, one product surface.** The tidy answer was one canonical stage model, and it would have been wrong for two of the three teams. Stages are per-team; only the surface is shared.
...
- **The assistant answers from filtered records, not free generation.** Efficiency questions have exact answers, so the assistant translates a question into a filter over stored records. A wrong answer is a wrong filter — visible and fixable — rather than a fluent invention.
- **Automatic stage classification with a visible correction.** Classifying unstructured tasks is the only way this works at volume, and it will sometimes be wrong. Every stage carries a one-click correction, and the correction rate is what I report.
...
Cutting automatic stage inference for RTL was the hardest call — the team with the most tasks and the most to gain. On 200 sampled tasks the classifier reached 54% against 71% elsewhere, low enough that leads would stop trusting the view. A manual stage field on task creation costs their lead ten minutes a week, and a slower correct view beats a fast one that needs checking.

#### 05. How I got it agreed

The RTL lead did not want to be in this product at all. He had built his own verification tracker in a spreadsheet that fit his flow exactly, and a shared tool would either flatten his stages into something meaningless or force his team to maintain data for somebody else’s dashboard — both had happened to him before.

I stopped arguing that a shared tool would serve him and asked what his spreadsheet could not do. The answer was comparison: he could not answer a director’s question about RTL against PS without a week’s notice. That reframed it to “what must be shared for comparison to work” — stage entry and exit timestamps, not stage names.

He agreed on two conditions. RTL keeps its own stage vocabulary, which is why the product carries three pipeline definitions. And nothing writes back to the task tool, so his team can never be blocked by a bug in mine — which is why write-back sits in the cut column, not the deferred one.

#### 06. What was built

A tracker showing where each task sits in its team’s pipeline, with a delay mode for overdue work, a drill-down drawer for one task’s history, a velocity leaderboard, and an assistant answering efficiency questions against stored records. Stage classifications are stored, so history survives.
...
Before build we agreed this failed if fewer than half of the 31 leads ran their weekly review inside it within a quarter. The guardrail was the stage correction rate — classification people quietly stop believing is worse than none, and the correction rate is its only early signal.
...
**How we counted.** A lead counts as running their review in the product when they opened their pipeline view in the 24 hours before a scheduled review, at least three weeks in four. Three populations on this page are not interchangeable: the three team leads running the Monday reviews observed in section 02, the 31 group leads forming the adoption denominator, and the 11 people in the January calendar sample. And 71% accuracy beside a 4.2% correction rate is not leads missing errors: accuracy was sampled across every task, while a lead only touches the stages their review passes through — why the correction rate, not accuracy, is the guardrail.
...
I treated the stage model as a data problem and it was a political one. Two teams changed their stage names once they saw them rendered publicly, quietly invalidating a month of classifier tuning. I would now freeze stage vocabularies with each lead in writing, and treat a change as a schema change with notice rather than a mysterious accuracy drop.
...
The register lost on a rate we had already measured: things people must remember to fill in get filled in about one time in eight. Minutes alone lost because they answer “what was decided”, not “what exactly did they say about the coupling test” — the question people actually brought.
...
The reframe: this was not a video problem. People wanted a passage of speech and were handed a timeline; every workaround they had invented was a way of getting speech back out of a medium that stores it badly.
...
- **The transcript is the navigation, not a feature beside the video.** Every line is a seek target, and the player is the secondary surface. That is what moved time-to-moment by minutes rather than seconds.
- **Both sources under one experience.** Recordings and stored video live in different systems and are the same object to someone searching. Unifying them removed a decision the user should never have had.
- **Full transcript context, not chunked retrieval.** A meeting is one argument spread over an hour, and chunking loses the thread that makes an answer correct. Whole-transcript context costs latency and buys answers that survive checking.
...
The compliance lead wanted recordings out of search entirely, and her concern was correct: a recording is a verbatim record of people speaking informally, indexing makes every offhand remark permanently discoverable, and she had no appetite for telling an employee that a sentence they had forgotten saying surfaced in a search two years later.

I could not argue the risk was imaginary, so I made the argument smaller. I separated the two things being indexed — that a meeting happened, and what was said inside it — and showed most of the retrieval value in our nine interviews came from finding the right meeting, which needs only titles, attendees and dates. A question of principle became one of scope.
...
What makes it worth having is opening one: every item carries its own transcript, suggested questions and minutes, and an answer comes back as the line a person said, timestamped.
...
Before build we agreed this failed if fewer than a quarter of re-opened recordings were entered by search or transcript jump rather than a direct link — a nicer player, not a way into the archive. The guardrail was access: any recording served outside its attendee list would stop the rollout.
...
**How we counted.** Time to the relevant moment means the interval between opening a recording and the last seek before two uninterrupted minutes of playback, excluding sessions with no playback at all — people confirming a meeting exists rather than looking inside it. The 140 in the weekly-use denominator is the mid-2025 roster, up from about 75 a year earlier, roughly flat since.
...
I built minutes generation before I understood who was supposed to send them. The number moved, but 11% of meetings having published minutes hides that a handful of people publish nearly all of them — the archive’s concentration problem moved one step downstream. A tool cannot assign ownership; I spent a quarter behaving as though it could.

I would also set the cache differently. Five minutes protected an API quota with plenty of headroom, and the cost lands on the most common first action — looking for the meeting you just left — where the product appears broken. I optimised a constraint I assumed rather than measured.
...
**What I did.** I owned a service that mirrors Outlook meetings to a Gmail attendee, publishes a subscribable feed, and reflects Google busy time back as opaque blocks. The product argument was restraint: what it may write, to whose calendar, and what it must never reveal.
...
It was also the only one where the risk of getting it wrong exceeded the prize: every incorrect write sends email to everyone invited.

#### 02. The problem as people experienced it

Six people kept a structured diary for two weeks, logging every time the calendars disagreed. A diary beat interviews because the incidents are individually forgettable — people said "a bit annoying" where the log showed something more expensive.
...
The reframe: they did not need their calendars merged, which is what they asked for. They needed each calendar to be right about when they were busy — and the diary’s last row, people who had stopped sharing calendar links over what titles revealed, turned a sync problem into a disclosure problem.

#### 03. My role and approach

I owned scope, the API surface, and when the service may modify anything. On a product where every write sends email, the interesting decisions are all refusals.

**Key decisions**

- **Busy blocks carry no detail at all.** Reflected time is an opaque block with no title, attendees or body. The diary had shown people self-censoring their calendars; a sync that leaks titles would make that worse.
...
- **Occurrence level, never the series.** Recurring meetings are where calendar automation does catastrophic things. Occurrence-level edits mean a bug damages one meeting, not a year of them.
- **Two paths depending on who owns the meeting.** Where the mailbox organises, the service adds an attendee; where it does not, it forwards. Attempting the first everywhere is the obvious implementation, and it fails silently on the meetings that matter most.
...
Two-way body and attendee sync was the hardest cut — it is what people asked for by name. It failed on a rule already set: a body sync writes meeting content into a tenant with a different compliance boundary. Opaque busy blocks plus the feed cover the gap: anyone needing detail subscribes deliberately and owns the disclosure.

#### 05. How I got it agreed

The IT security administrator refused the design on first review, and he was right to. Unattended operation needs application-level permissions, and as scoped those grant read and write across every mailbox in the tenant. His question: why does a calendar tool for eleven people need access to the CEO’s mailbox.

I had no good answer, which was the useful part. Rather than argue likelihood of misuse, I found the mechanism — an access policy restricting the application’s reach to a named mailbox group — and came back with the scoped version, demonstrated to fail cleanly outside the group. The conversation stopped being about trust and became a configuration we could both read.

He approved with two conditions. IT manages the mailbox group, not me, so I cannot widen the service’s reach by editing my own config. And every write is logged with actor, target and reason for a year — a log that has already cleared the service twice, in incidents that were not its fault.
...
We agreed two failure conditions before build, one absolute: any private title reaching the Google side stops the service rather than triggers a fix. The second: double-bookings reach zero within a month. Disclosure was the guardrail throughout — the headline can be met perfectly while the thing that matters fails.
...
**How we counted.** A double-booking means two accepted meetings overlapping by more than five minutes on the same person, counted from both calendars, not complaints — the diary showed most clashes absorbed silently. The two figures on this page count different things: the headline six a month is the externally visible rate the quarter was prioritised on; the diary’s eleven in a fortnight counts every clash, silent ones included. And the Outlook tenant this service reads is the smaller of the group’s two — twelve mailboxes — which is why narrowing the credential reads 12 → 1 rather than a company-sized number.
...
I scoped this as a sync problem for a month before the diary told me it was a disclosure problem — a month spent designing category filters and a preferences page, all assuming people wanted to choose what crossed over. They wanted to be confident nothing did. One line in one log got me there: they had stopped sharing calendar links.
...
**Result.** Early signal only. What is structural: a figure with no dated source does not render at all, the evidence floor refuses a role rather than widening a band, and no figure anywhere comes from a model — press any number in the demo and its citation opens. The pilot counts below are the worked example.
...
Timing carried the rest: pay transparency filings had made posted ranges usable — a third independent source where a year earlier there were two.

#### 02. The problem as people experienced it

I read the last 26 closed requisitions and tried to trace every number in each agreed range to something with a date on it, then timed four live offer conversations for how long each spent on where a figure came from.
...
Flagging existing employees was the hardest cut: nearly free once the bands exist, and the first thing everyone asks for. It fails on what the data is. Comparator rows describe a market and a level; the gap between a person and a band is explained by tenure, scope and a hiring market from three years ago. A list of names ordered by band position would be read as a finding — it is a rendering of what we do not know.
...
So I did not argue it. I proposed instead that a refusal be worth more than the band: it names the rule that failed, the evidence against it, and what would clear it — a task rather than a closed door.
...
Eighteen drafts is enough to see whether the citation rule survives a live hiring round, and far too few to say anything about pay.
...
Two softer gates run to the end. If refusals stay above half of requisitions at 40 drafts, the floor is set for a data estate we do not have — buy data rather than move rules. If offers outside the range pass a fifth, the range is not what is being used.

#### 10. What I'd do differently

I built the weighting table as a constant and treated its numbers as obvious. They are not: the weight on self-reported data is an afternoon judgement I never tested, and the band moves with it. It should have shipped as a versioned assumption.
...
A workflow taking a structured brief through five stages: context validation with a clarification gate, project-type classification and section selection, batched drafting, scorecard review by a different model with a bounded revision loop, and one implementation-ready task whose checklist carries binary completion tests, estimates and dependencies. Around it: versioning, line-level diffs, export and comment-driven revision.
...
We agreed before build that this failed if humans approved fewer than half of plans on first read — a threshold deliberately about the reviewer: a loop that passes work a person then redoes spends the model’s rounds and the human’s attention. Model independence was the guardrail: any run where planner and reviewer shared a model invalidates the approval, enforced in code and counted.
...
I scoped the pilot to the US path: mixing two legal standards puts a jurisdiction condition in every prompt.
...
The reframe: the hard part was never the agent lineup — it was retrieval, containment, and telling an inventor what their disclosure fails to teach. The third turned out to be worth shipping on its own.

#### 03. My role and approach

I owned scope, build order and the operating rules. Build order is the contribution: what cannot be retrofitted first, and a measurement ahead of the work it would justify.

**Key decisions**

- **Containment at intake, and inside every outbound client.** Enforced at the call boundary, not at client construction: both clients memoise their SDK object, so a construction-time check fires once and enforces nothing after.
- **Measure the corpus ceiling before tuning a ranker.** We agreed before the eval existed that a ranker within 85% of achievable was not the thing to work on. Three weeks answered what would otherwise take months.
...
**What I did.** I owned an in-tenant notetaker: capture from the meeting’s own recording artefact rather than a guest participant, cued transcripts, and minutes where every claim cites the second it was said. Organisers exclude a meeting before it starts, irreversibly.

**Result.** Nothing leaves the tenant and no bot joins a call. A claim without a timestamp does not ship — the rule that made minutes circulable. The decision-trace figures below are the worked example.
...
Read that way it stops being build against buy: the vendor product is better than anything two engineers will write, and it may touch a third of the estate. Archive retrieval has its own page; this one is how the record gets made.

#### 02. The problem as people experienced it

I ran a decision trace rather than a survey: 24 decisions someone had asked about again the following month, traced to wherever each was written down. Asking whether meetings are well minuted gets an opinion; asking where a decision is recorded gets a file path or a shrug.
...
The reframe: nobody was choosing not to write minutes. Writing them costs the most engaged person twenty minutes right after the meeting, and that person is already late for the next call. Anything that starts by asking a human inherits that arithmetic.

#### 03. My role and approach

I owned capture, consent and what generated minutes may claim. The judgement was what the output must never do: a wrong minute is worse than none, and read by more people.

**Key decisions**

- **No bot ever joins the call.** Capture reads the meeting’s own recording artefact after the fact. Unlike a guest participant it is simply there, and cannot change who feels able to speak.
- **A claim without a timestamp does not ship.** Every line cites the second it came from or is dropped. This moved minutes from written to sent: nobody circulates a claim they cannot spot-check.
- **Exclusion is pre-call and irreversible.** A control you can lift afterwards is a request someone can be pressured into granting. The organiser’s mark is the only consent surface.
- **An excluded meeting stays in the library, named.** Listed with its exclusion stated, never hidden. A silent disappearance sends people back to asking the organiser.
...
The guest-bot fallback was the hardest cut, and the one I argued for before dropping it. Meetings on a customer’s platform expose no recording artefact, so the only way in is the thing the product exists to avoid — an exception made once is the argument conceded. About nine a month stay uncovered, none internal, and the organiser is told plainly nothing was captured.

#### 05. How I got it agreed

The head of engineering wanted to buy, and on everything measurable he was right: the vendor won the bake-off on word error rate, badly on diarization, needed no engineers, and was available that afternoon. His answer to the residency worry was the strongest there is: the tenant already runs on somebody else’s hardware, so my line was contracts, not physics.

So I stopped arguing the principle and tested the control: a vendor bot restricted to organiser-cleared meetings. Over four weeks organisers cleared 66 of 212 — and legal review found 4 of the 66 had discussed unfiled disclosures, cleared in good faith before an agenda existed to read.

That ended it: a control failing 6% of the time fails on the exact cases it exists for, and one failure costs a filing. We built, on his conditions: the diarization gap is published monthly against the vendor, reopening the decision had we still trailed by June.

#### 06. What was built

A library of recordings; behind each, a workspace: transcript, generated note, and a question box answering from that recording only. Capture runs from a change-notification subscription on tenant call records; transcription and summarisation on tenant-resident inference; nothing crosses the boundary.

The note is where the judgement went. A reader a month later wants the decision and why, the figure they may quote with its conditions, and what was left unsettled — so the note is those sections, in that order, a timestamp on every claim.
...
The last row loses most and I defend it hardest: a meeting whose value is the room going quiet on a number produces minutes that say nothing about it, because no single second carries it. We publish thinner minutes rather than minutes the transcript cannot support.

#### 08. Impact and outcomes

We agreed before building that this failed below 95% transcription of eligible meetings sustained over a month — silent gaps invite confident wrong answers about meetings never captured. The residency guardrail was absolute: one recording processed outside the tenant ends the project, so the vendor client was never installed rather than installed and disabled.
...
| name | a | b |
|---|---|---|
| Durable written record | 5 | 21 |
| Chat thread, had to be located | 7 | 2 |
| Somebody's memory only | 12 | 1 |

_The January trace repeated on 24 fresh decisions from July, Aug 2026 — same method, same coder. The middle row is the honest one: chat threads did not disappear, they became the place a link to the note gets posted. The one decision still living in memory was made in a corridor, and no capture path fixes that._

**How we counted.** Eligible excludes meetings the organiser marked before the call. Transcribed means a cued transcript exists, not a recording file — the two arrive by different paths, and counting files would have called the lapsed-subscription week healthy.
...
I measured diarization in week nine, after the minutes’ shape was set. It came back worse than assumed above three speakers, and the fix weakened the product: attributed claims were dropped, because a wrong attribution is worse than none. That measurement belonged in week one.

I would also build the correction path first. There is still no way to fix a wrong claim except regenerate and hope — I treated correction as a feature rather than what makes an automated record trustworthy. Six people have asked; nothing else comes close.
...
A dashboard over a nightly sync. The sync enumerates workspace documents, checks each for modification, fetches and hashes content only where something may have changed, and writes an immutable version when the hash differs. The interface: a repository view, a per-document version timeline with diffs, a change feed, and a run history showing what was checked and what changed.
...
We agreed the MVP failed below 95% sync completion over a month — a change record with gaps is worse than none, inviting confident wrong answers about what did not change. History loss was the guardrail: after the policy reversal, any document dropped from tracking would repeat the failure just fixed, so pruning was removed from the codebase rather than disabled.
...
I would also have built owner notifications before the dashboard. The eleven document owners still learn their document changed by visiting a page they have no reason to visit. Everything needed for a notification exists; I built the observation surface first because it was what I could see — the delivery surface is where the value lands.
...
Cutting the matched-pair product is what set the tier, and it still costs us sales. A pair has to match in more than level: the trim makes two microphones agree at 1 kHz and says nothing about 8 kHz or thirty degrees off axis — exactly what a stereo pair is for. Selling pairs honestly meant screening for response shape as well as sensitivity: a second rejection criterion, and inventory held while a partner is found for each unit. We ship singles, and the review that costs us most says the microphone is very good and cannot be bought twice.

#### 05. How I got it agreed

The acoustics consultant told me the trim was dishonest, and he had a real argument. I was calibrating the number we print, at one frequency, against a reference — and a capsule wrong in the shape of its response can be trimmed to report correct sensitivity and still sound wrong. His phrase: I had built a device for making the datasheet true.
...
The envelope test checks five frequencies, chosen because our rig could sweep them quickly. A capsule can pass all five and misbehave between them — precisely the failure the envelope exists to catch — so the test samples the argument rather than making it. A continuous sweep against a mask would have cost seconds per unit on a bench already running.
...
Closed-loop tool emulation was the hardest cut, and it is what most people picture. Modelling a tool well enough to respond to its own outputs meant firmware models we did not have, and it fails the actual requirement: a loop introduces its own dynamics, so the same fault does not land the same way twice. Open-loop playback covers it — a fault the analyst can trigger, repeat and instrument; where genuine loop behaviour matters they still put the tool on a rig. The honest cost: interaction faults, where two channels misbehave together, stay outside the bench and outside the 56.

#### 05. How I got it agreed

The metrology and calibration owner would not let the bench into the labs. Her objection was not accuracy in the abstract: a lab that signs findings uses instruments with traceable calibration, and I was proposing to feed them a signal from a laptop and a data-acquisition module with no certificate behind it. A failure attributed on the strength of my waveform inherits my uncertainty, and nobody has recorded what it is.
...
**What we learned.** The disguise question was tested before the flight-time question, deliberately — if it does not read as a bird, a longer flight buys nothing, because better aeroplanes exist. It flew stable circuits and never approached the club’s own written targets of an hour and twenty-five kilometres. The observer-trial and flight figures below are the worked example.
...
Flapping-wing surveillance is a funded category: DARPA paid for the Nano Hummingbird — twenty grams, a camera, about ten minutes — and India has run a national micro-air-vehicle programme for a decade. A club can still test the property those programmes chase.
...
So we tested the disguise before the aircraft: flew it past people who had not been told what to expect, and asked what they were looking at.
...
I was project manager for this build inside SRM UAV. The club owned the craft — fabrication, wing membranes, the gear train, the flying. I made it scoped and pitchable: a customer class, one property worth proving, and a number that would stop us.

None of that is a title. It is deciding a disguise test comes before a flight-time record, and holding to it when the interesting work was the gearbox.
...
A single-motor flapping-wing airframe: a gear train turning rotary motion into symmetric flapping on carbon spars, a fuselage holding the electronics, a tail doing the steering. The render below is the concept; what flew was cruder.
...
We cleared our own bar and the bar was wrong: forty-one seconds beats thirty and means nothing against a concept written around an hour aloft — a build milestone wearing a product criterion’s clothes. The campaign showed where the ceiling sits: the gear train wears, payload margin is a few grams, and control authority runs out before endurance does.

We pitched the concept to the Defence Research and Development Organisation; nothing here describes what came back, because nothing is recorded. Preparing it taught us something anyway: naming a customer class forced us to admit that of seven surveillance tasks in our brief, only two survived contact with forty-one seconds. It never went to users — no operator flew it.
...
I would have put load cells on the bench rig before the first airframe, not after the fourth. We read the rig by eye, tuning the gearbox by whether it sounded right — four structural failures are what that cost.
...
The answer is not close. Carbon positive needs net removal, and nothing on this vehicle removes carbon dioxide — the filters clean particulates and odours, a real public good and an entirely different molecule. The turbine harvests drag the motor has already paid for. What it changed is the claim: we stopped saying carbon positive, dropped the turbine, and called the filters air quality rather than carbon.
...
I would also separate the two good ideas from the bad one earlier. Solar on a car and a filter stack are both defensible, and they spent a year attached to a turbine that was not — so when the turbine failed, the whole concept looked discredited rather than two thirds intact.
...
With the GNSS module already in our own component list, positioning contributes about 1.4 dB against a 0.5 dB pass mark. The method fails as proposed — and that is the finding, because it arrived before anyone had bought a motor. What it changed: precise-point positioning became the first deliverable, and the aerodynamic work the team wanted to start on moved behind it.
...
We decomposed four experiments because four was what we could get full error reporting from — a thin base for a conclusion we let redirect the whole programme. Three of the four share a research lineage, so their agreement is weaker evidence than it looks; we did not press the point, because the answer was one we liked.
...
Cutting the number plate is what turned this from the project we wrote up into the thing that exists. Plate reading impressed everyone, and it changes what the system is: once a plate is extracted, a count becomes an accusation about a named student, delivered automatically, with no appeal path and a detector 41% accurate after dark. We had no mandate to build that and nobody to answer for it when wrong. The survey number is defensible precisely because it identifies nobody — the cost is that the system cannot deter a single rider.
...
I had been arguing that our detector was good enough to be fair, which conceded his frame entirely. His question was not whether we would be right but who owns the consequence when we are wrong. So I re-specified the output: counts per clip, no plates, no faces, nothing that resolves to a person — nothing for anyone to be custodian of.
...
We never measured how often our two labellers disagreed with each other, so we have no floor to compare 94% against. If humans agree only 95% of the time on a dusk clip, the model is already at the ceiling. That number would have cost an afternoon.
...
**What we did.** We tested whether a tracked drive could cross settled sludge without fluidising it — one lane, three graded analogues — agreeing in advance that a vehicle needing to be pulled out on its tether had failed.

**Finding.** A track is a machine for not leaving the medium alone: it shears, shear-thinning sludge thins where sheared, and the vehicle digs its own hole. So the final design carries its weight in a buoyant frame and pushes with paddles and thrusters. The threshold and tank runs below are the worked example.
...
Indian sewer networks were not built to be walked through; inspection still often means lifting a manhole and sending somebody down. A machine displaces that practice only if it can get from one manhole to the next unaided.
...
The beneficiaries are municipal sewer and sump inspection crews — no buyer, just a job done by people who should not be doing it.

#### 02. The problem as people experienced it

We built a test tank and mixed three sludge analogues to bracket what a settled line contains, then ran the same drive down the same lane in each — the medium varied, the vehicle stayed fixed.
...
The reframe: we had treated sludge as a difficult floor. It is a shear-thinning fluid that behaves like a floor only while left alone — and a track is a machine for not leaving it alone.

#### 03. My role and approach

I founded Ricky Kids and led the team into the Grand Challenge; a faculty mentor signed the institutional side, and I decided what we would test and what counted as failing.

The judgment worth pointing at is spending the first quarter on a tank rather than a vehicle. The project name assumes traversing is possible; the cheapest thing to do was try to disprove our own title.
...
- **Make self-recovery the pass mark, not distance.** A vehicle that travels ten metres and then needs a person to fetch it has added an entry to the job.
...
The tank came first; the vehicle below is what it produced — a sealed acrylic cylinder for the electronics, slung inside a closed loop of PVC pipe serving as both frame and buoyancy.
...
Tracks completed the lane in the settled analogue and nothing else, and seven of nine tracked stalls ended with the vehicle pulled out. The mechanism is not weak motors: a track shears the medium under itself, shear-thinning sludge thins where sheared, and the vehicle digs its own hole. We stopped designing a crawler and designed the thing above.

What this could not tell us is how any of it behaves in a real sewer. We tested clean analogues in a straight tank: no rag, grit, roots, fat, joints to snag on, no long-duration fouling of seals or thrusters. Each is a plausible way for this vehicle to fail, and we can say nothing about any of them.

**How we counted.** Traverse success is the share of runs completing the full lane under the vehicle’s own power in a given medium, excluding runs aborted for tether tangles — a property of our tank, not the drive. Stalls are counted as events, not runs — a run can stall twice, which is how nine tracked runs produced nine stall events — and a stall counts as recovered only if forward motion resumed without tether tension.
...
We mixed our analogues to a density because density is easy to measure with what we had. But yield stress, not density, governs this — two media at the same density can behave differently under a track. Our threshold is a proxy a running track eats into: the part-settled analogue sat 0.03 g/cm³ inside the band and still failed; the margin above 1.15 matters, not the line.
...
What this could not tell us is whether any of it survives contact with a municipality. Nothing was built and no block was instrumented: the model uses published averages, assumes utilities that are in practice three procurement bodies with three budgets, and ignores capital cost. The planners we spoke to said coordination, not physics, kills schemes like this.

**How we counted.** Saving from coupling is the modelled reduction in a block’s annual resource use — energy, potable water and landfilled waste on a common energy basis — when the three run as one loop rather than independently. It excludes capital cost, construction carbon and any benefit outside the block; the interval is the span from moving every assumption across a defensible range, not a statistical confidence interval.
...
The audit should have come before the paper. Everything here would have improved the publication rather than qualifying it; dropping a claim in a draft costs nothing, dropping one already presented costs a page like this.

I would also have gone to the planners first. We spent two quarters establishing that coupling is physically worthwhile in housing, and the first planner we met said physics was never the obstacle — three utilities with three budgets and three procurement cycles is.
